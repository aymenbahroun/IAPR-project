...
def digit_class(reg, im):
	...
	##this part is for the rotated version!!!!
	M = measure.moments_central(padded, order=2)
	padded = rotate(padded, -0.5*np.arctan2(2.*M[1,1],M[2,0]-M[0,2])/math.pi*180., reshape=False, order=3)
	##--------
